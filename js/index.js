...
//init data
var alreadySetSysHostId = false; //如果用户双击了一个host节点，就设置那个节点的id给它
...
$baseHostBtn.on("click", function(){
  $hostArea.focus().val( hostContentOp.get("base-host") );
  ...
  //data-writeSys : 1表示 textarea 输入需要写进系统，0则不需要
  $hostArea.attr({
    "data-id" : id,
    "data-title" : title,
    "data-writeSys" : alreadySetSysHostId ? 1 : 0
  });
});
...
$nodeList.on("click", "a", function(){
  ...
  $hostArea.focus().val( hostContentOp.get(id) );
...
$hostArea.keyup(function(){
  ...
  hostAreaChangeTime = setTimeout(function(){
    ...
    if( writeSys == 1 ){
      ...
      if( id == "base-host" ){
        writeHostIntoSys( hostContentOp.get(alreadySetSysHostId) );
      }else{
